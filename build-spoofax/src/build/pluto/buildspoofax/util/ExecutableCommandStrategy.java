...
public class ExecutableCommandStrategy extends xtc_command_1_0 implements Externalizable {
		...
		public static ExecutableCommandStrategy getInstance(String command, Path exe) {
			...
			if (!strategy.exe.equals(exe))
				throw new IllegalArgumentException("Existing strategy for command " + command + " has different executable. Existing: " + strategy.exe + ". This request: " + exe);
			return strategy;
		}
		...
		private ExecutableCommandStrategy(String command, Path exe) {
			init(command, exe);
		}
		
		private void init(String command, Path exe) {
